...
mpv_extra=(["--vo=gpu","--gpu-context=drm"] if headless else [])
...
while True:
    ...
    if v:
        print("Playing:", v)
        rc=subprocess.run(["mpv","--fs","--no-border","--loop-file=inf","--input-default-bindings=yes",f"--input-conf={conf}",*mpv_extra,str(v)]).returncode
        if rc==0: break
    time.sleep(1)
